...
- 表示値: 年平均、日最高・日最低気温の日別気候平均、月別グラフと数値表
...
同時にDaily APIから1991年1月1日〜2020年12月31日の`T2M_MAX`と`T2M_MIN`を取得し、各暦日について有効値を平均します。通常日は30年分、2月29日は8年分です。API応答が欠けた日は集計から除き、日別データだけ取得できない場合も月別・年平均値は表示します。

選択枠は操作用の1°×1°格子で、白い小点は取得する中心点です。値は円内または選択枠全体を面積平均したものではなく、枠中心を含むPOWER元データ格子の代表値です。POWERの公表解像度は、気象要素が約0.5°×0.625°、日射要素が1°×1°です。
