...
世界地図をクリックし、日本では気温・降水・日射の約1km独自推定値、それ以外ではNASA POWERの1991–2020年気候平均を表示する公開試作ツールです。相対湿度は日本でもNASA POWERです。

- 公開URL: `https://nature-wx-lab.github.io/plant-climate-mesh/`
- 表示要素: 地上2m気温、補正済み降水量、全天日射量、地上2m相対湿度
- 分布図: 日本の気温・降水・日射は約1kmの推定値を広域では概要画像、拡大時には元格子から描画。湿度と日本以外は従来のNASA POWER表示
...
日本以外と湿度の全球分布図はNASA POWER Data v10の公開AWS Zarr月別データから事前生成した静的PNGです。ブラウザからAWSへ接続せず、選択された要素・期間の画像だけを同一GitHub Pages配信元から読み込みます。
...
日本の気温・降水・日射は、約1km格子の月別・年別数値を別に配信して重ねています。ズーム32未満は2048pxの概要画像、それ以上は表示範囲に必要な1km格子の数値を読み込み、画面解像度で再描画します。広域画像は視覚的なプレビューで、地点値は画像の色から逆算せず日別格子値を読み込みます。透明な日本の部分だけを上書きし、NASAの色と混ぜません。

### 日本の約1kmメッシュ

対象は日本の標準3次メッシュ387,717格子。気象庁の1991–2020年の日別平年値と既存の月別約1kmメッシュを組み合わせて推定した、年内366日分の気温（平均・最高・最低）・降水量・全天日射量を使用します。独自推定値であり、気象庁が公表する公式な「1km日別平年値」ではありません。月平均の形状制約により日別の実測値を復元したものでもありません。日別平年値が存在しない格子は欠測として扱い、NASA値で埋めません。相対湿度の1km値は存在せず、日本でもNASA POWERを使います。

公開用の派生数値は4桁のメッシュ接頭辞単位に圧縮し、クリック時は当該区画の5要素だけ取得します。公開先には観測所情報や作成時のローカル情報を含めません。地図用の月値は通常年の365日で集計し、2月29日を除きます。降水量の分布図だけは着色のため整数mmに丸め、地点の月・年表示は日別の0.1mm値を合算します。日射の推定に使える観測所が少ない地域、地形・沿岸・都市の細かな差には不確実性があります。最低気温は既存の月別1km最低気温メッシュに合わせて再計算し、最低値が同じ格子・暦日の平均値を上回らないことを公開前に全件検査します。これは気象庁の公式1km日別値や局地的な実測精度を保証するものではありません。

### 地点詳細

日本以外では地点選択時だけNASA POWER Climatology APIへ問い合わせます。日本では湿度のみ同API・Daily APIへ問い合わせます。
...
日本以外の橙枠は、クリック位置に最も近いMERRA-2格子中心へそろえた約0.5°緯度×0.625°経度の気象格子です。気温・補正済み降水量・相対湿度はその元格子の空間平均として扱います。日射量は同じ中心座標に対応する別の1°×1°格子です。日本の選択枠は約1kmの標準3次メッシュですが、湿度だけはその中心点付近のNASA格子値です。
